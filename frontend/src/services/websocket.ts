...
export class VoiceTranscriptionClient {
  ...
  sendAudioChunk(audioData: ArrayBuffer): void {
    if (!this.isConnected || !this.ws) {
      console.warn('⚠️ WebSocket не подключен, пропускаем аудио чанк')
      return
    }

    if (audioData.byteLength === 0) {
      console.warn('⚠️ Получен пустой аудио чанк, пропускаем')
      return
    }
  ...
  endTranscription(): void {
    if (!this.isConnected || !this.ws) {
      console.warn('⚠️ WebSocket не подключен для завершения транскрипции')
      return
    }
...
export const getGlobalWebSocketClient = (
  ...
  if (!globalWebSocketClient) {
    ...
    console.log('♻️ Переиспользуем существующий WebSocket клиент')
